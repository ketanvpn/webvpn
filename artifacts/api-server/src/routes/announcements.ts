...
router.get("/admin/announcements", requireAdmin, asyncHandler(async (_req, res) => {
  const rows = await db
    .select()
...
router.post("/admin/announcements", requireAdmin, asyncHandler(async (req, res) => {
...
router.put("/admin/announcements/:id", requireAdmin, asyncHandler(async (req, res) => {
...
router.delete("/admin/announcements/:id", requireAdmin, asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id as string, 10);
...
router.get("/announcements/active", requireAuth, asyncHandler(async (_req, res) => {
  const now = new Date();
  const rows = await db
    .select()
    .from(announcementsTable)
