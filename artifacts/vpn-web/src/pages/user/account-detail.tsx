import { useGetAccount, useGetBalance, getGetAccountQueryKey, getGetBalanceQueryKey } from "@workspace/api-client-react";
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  ...
                  <CardDescription className="mt-1 flex min-w-0 items-center gap-2 text-sm">
                    ...
                    <span className="min-w-0 truncate">{account.server?.name ?? "Server lama/tidak tersedia"} &bull; {account.server?.location ?? "-"}</span>
                  </CardDescription>
                </div>
              ...
              {account.isActive && new Date(account.expiresAt) > new Date() && (
                  isDynamicAccount ? (
                    dynamicOrder?.renewEnabled !== false && dynamicRenewTypes.length > 0 ? (
                      <DynamicRenewDialog
                        ...
                        serverName={account.server?.name ?? "Server lama/tidak tersedia"}
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="text-xs text-muted-foreground text-center">
                      Perpanjangan paket lama tidak lagi tersedia. Silakan buat order baru untuk memperpanjang akun.
                    </p>
                  </div>
                )
              )}
