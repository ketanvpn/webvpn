...
interface ResellerPromoSectionProps {
  promo: PromoData;
  promoRequesting: boolean;
  promoRequested: boolean;
  onPromoRequest: () => void;
}

export function ResellerPromoSection({
  promo,
  promoRequesting,
  promoRequested,
  onPromoRequest,
}: ResellerPromoSectionProps) {
  ...
  return (
    ...
    >
      {/* Dekorasi */}
      <div
        className={`absolute -right-6 -top-6 h-24 w-24 rounded-full blur-2xl opacity-50 ${
          isAuto ? "bg-green-200" : "bg-primary/20"
        }`}
      />
      ...
      <div className="relative px-5 pt-4 pb-5 space-y-3">
              ...
              <li key={i} className="flex items-start gap-1.5">
                <CheckCircle2
                  className={`h-3.5 w-3.5 mt-0.5 shrink-0 ${
                    isAuto ? "text-green-500" : "text-primary/70"
                  }`}
                />
        ...
        {/* Syarat */}
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-wide text-orange-600 dark:text-orange-400 mb-1.5">
            Syarat
          </p>
          <ul className="space-y-1">
              ...
              {isAuto ? (
                <span className="text-xs text-muted-foreground">
                  ...
                  <span className="font-semibold text-foreground">
                    {formatRupiah(promo.autoUpgradeMinTopup)}
                  </span>{" "}
                  — langsung otomatis aktif sebagai reseller
                </span>
              ) : (
                <span className="text-xs text-muted-foreground">
                  Ajukan permintaan ke admin untuk diaktifkan
                </span>
              )}
            </li>
            {promo.targetEnabled && (
              <li className="flex items-start gap-1.5">
                ...
                <span className="text-xs text-muted-foreground">
                  Wajib jual minimal{" "}
                  <span className="font-semibold text-foreground">
                    {formatRupiah(promo.monthlyTarget)}
                  </span>{" "}
        ...
        {isAuto ? (
          ...
          <p className="text-xs text-muted-foreground">
            Hubungi admin untuk bergabung sebagai reseller.
          </p>
        )}
      </div>
    </Card>
