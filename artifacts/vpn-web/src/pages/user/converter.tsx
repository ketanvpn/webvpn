...
import {
  ...
  convertVmess,
  writeClipboard,
} from "./converter/converter-utils";
import { HttpCustomGuideCard } from "./converter/http-custom-guide";
import { PaketOnboardingGuide } from "./converter/onboarding-guide";
...
export default function ConfigConverter() {
  ...
  const requestedAccountId = Number(new URLSearchParams(search).get("account") ?? "0");

  // Derived state
  const selectedEasyPreset = useMemo(
    () => (presetsError ? null : easyPresets.find((preset) => String(preset.id) === easyPresetId) ?? null),
  ...
  const unknownAccounts = useMemo(
    () => activeSshAccounts.filter((account) => classifySshAccount(account) === "unknown"),
    [activeSshAccounts],
  );
  const compatibleAccounts = useMemo(
    () => (selectedEasyPreset ? activeSshAccounts.filter((account) => isAccountCompatibleWithPreset(account, selectedEasyPreset)) : []),
    [activeSshAccounts, selectedEasyPreset],
  );
  const selectedEasyAccount = compatibleAccounts.find((account) => String(account.id) === easyAccountId);
  const httpCustomGuide =
    easyApp === "http-custom" && selectedEasyPreset && selectedEasyAccount
      ? buildHttpCustomGuide({ account: selectedEasyAccount, preset: selectedEasyPreset })
      : null;

  // Effects
  useEffect(() => {
    if (initializedFromQuery.current || accountsLoading || presetsLoading || !requestedAccountId) return;
    initializedFromQuery.current = true;
    const requested = activeSshAccounts.find((account) => account.id === requestedAccountId);
    if (!requested) return;
    const compatiblePresets = easyPresets.filter((preset) => isAccountCompatibleWithPreset(requested, preset));
    setEasyAccountId(String(requested.id));
    if (compatiblePresets.length === 1) setEasyPresetId(String(compatiblePresets[0].id));
  }, [accountsLoading, activeSshAccounts, easyPresets, presetsLoading, requestedAccountId]);
  ...
  useEffect(() => {
    ...
    if (easyAccountId && !compatibleAccounts.some((account) => String(account.id) === easyAccountId)) {
      setEasyAccountId("");
      resetEasyApplicationState();
    }
  ...
  useEffect(() => {
    ...
    if (!appIsSupported || (previous !== null && previous !== signature)) resetEasyApplicationState();
  ...
  const syncAccountMutation = useMutation({
    mutationFn: (accountId: number) => apiClient.post(`/api/accounts/${accountId}/sync-provider`),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["my-ssh-accounts"] });
      toast({ title: "Data akun diperbarui", description: "Silakan pilih kembali paket dan akun yang sesuai." });
    },
    onError: (error: Error) => toast({ title: "Gagal memperbarui akun", description: error.message, variant: "destructive" }),
  });
  ...
  function selectEasyPreset(presetId: string) {
    ...
    const currentAccount = activeSshAccounts.find((account) => String(account.id) === easyAccountId);
    if (!nextPreset || !currentAccount || !isAccountCompatibleWithPreset(currentAccount, nextPreset)) setEasyAccountId("");
    resetEasyApplicationState();
    if (presetId) setEasyActiveStep(2);
  }

  function selectEasyAccount(accountId: string) {
    setEasyAccountId(accountId);
    resetEasyApplicationState();
    if (accountId) setEasyActiveStep(3);
  }
  ...
  function selectEasyApp(app: EasyApp) {
    ...
    } else if (app === "darktunnel" && selectedEasyPreset && easyAccountId) {
      const account = compatibleAccounts.find((item) => String(item.id) === easyAccountId);
      if (account) {
        try {
          const generated = buildDarkTunnelConfig({ account, preset: selectedEasyPreset });
  ...
  function generateEasyConfig() {
    if (easyApp !== "darktunnel" || !selectedEasyPreset || !easyAccountId) {
      toast({ title: "Pilih paket, akun, dan aplikasi", description: "Pilih preset aktif, akun SSH yang cocok, lalu DarkTunnel.", variant: "destructive" });
      return;
    }
    const account = compatibleAccounts.find((item) => String(item.id) === easyAccountId);
    if (!account) {
      toast({ title: "Akun tidak kompatibel", description: "Pilih akun yang ditampilkan pada daftar.", variant: "destructive" });
      return;
    }
    try {
      const generated = buildDarkTunnelConfig({ account, preset: selectedEasyPreset });
      ...
      toast({ title: "Config gagal dibuat", description: error instanceof Error ? error.message : "Data akun tidak valid.", variant: "destructive" });
    }
  }

  function downloadEasyConfig() {
    if (!easyResult) return;
    const blob = new Blob([easyResult.link], { type: "application/octet-stream;charset=utf-8" });
    ...
    toast({ title: "File .dark diunduh", description: "Buka file tersebut menggunakan aplikasi DarkTunnel." });
  }
  ...
  async function copyEasyLink() {
    ...
    try {
      ...
      toast({ title: "Gagal menyalin", description: "Gunakan tombol Download File .dark sebagai gantinya.", variant: "destructive" });
    }
  }
  ...
  const handleConvert = () => {
    if (!rawConfig.trim() || !selectedBugId) {
      toast({ title: "Data belum lengkap", description: "Masukkan config mentah dan pilih preset bug.", variant: "destructive" });
      return;
    }
    ...
    const convertedLines = lines.map((line) => {
      ...
      if (line.toLowerCase().includes("bug") || line.toLowerCase().includes("ssh") || line.includes("GET ") || line.includes("Host:") || line.includes("CONNECT ")) return convertSshOrText(line, bug) || line;
      return line;
    });
    if (convertedLines.every((line, index) => line === lines[index])) {
      toast({ title: "Gagal mengonversi", description: "Format config tidak dikenali.", variant: "destructive" });
      return;
    }
  ...
  const handleAdvancedSshConvert = () => {
    ...
    if (!bug?.sshInjectConfig || Object.keys(bug.sshInjectConfig).length === 0) {
      toast({ title: "Pilih preset yang valid", description: "Preset harus memiliki SSH Inject Config.", variant: "destructive" });
      return;
    }
    setIsSshConverting(true);
    const link = buildAdvancedDarkTunnelSsh(
      { host: sshHost.trim(), port: Number.parseInt(sshPort, 10) || 80, username: sshUsername.trim(), password: sshPassword },
      bug.sshInjectConfig,
  ...
  async function copyHttpField(id: string, value: string, label: string) {
    try {
      await writeClipboard(value);
      setCopiedHttpField(id);
      toast({ title: `${label} tersalin`, description: "Tempel ke field yang sama di HTTP Custom." });
      window.setTimeout(() => setCopiedHttpField((current) => (current === id ? null : current)), 2000);
    } catch {
      toast({ title: "Gagal menyalin", description: `Tekan lama nilai ${label}, lalu pilih Salin.`, variant: "destructive" });
    }
  }
    ...
    <div className="mx-auto w-full max-w-4xl min-w-0 space-y-6 overflow-hidden px-1 sm:px-0 pb-8">
      ...
      <Tabs defaultValue="easy" className="w-full min-w-0 space-y-5 overflow-hidden">
        ...
        <TabsContent value="easy">
          <EasyModeTab
            ...
            easyActiveStep={easyActiveStep}
            easyApp={easyApp}
            presetsLoading={presetsLoading}
            presetsError={presetsError}
            presetsQueryError={presetsQueryError instanceof Error ? presetsQueryError : null}
            presetsFetching={presetsFetching}
            accountsLoading={accountsLoading}
            accountsError={accountsError}
            accountsQueryError={accountsQueryError instanceof Error ? accountsQueryError : null}
            ...
            onSetActiveStep={setEasyActiveStep}
            onGenerateConfig={generateEasyConfig}
            onRefetchPresets={() => void refetchPresets()}
            onRefetchAccounts={() => void refetchAccounts()}
            onShowHttpModal={() => setShowHttpModal(true)}
          />
        </TabsContent>
        ...
        <TabsContent value="advanced">
          <AdvancedModeTab
            ...
            onSetSelectedBugId={setSelectedBugId}
            ...
            onSelectAccountForSsh={(account) => {
              const links = account.allLinks ?? {};
              setSshHost(links.cloudfront || links.domain || links.host || links.hostname || account.server?.originalHost || account.server?.host || "");
      ...
      <Dialog open={showHttpModal} onOpenChange={setShowHttpModal}>
        <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto p-2 sm:p-6 glass-panel border-cyan-500/30">
          {httpCustomGuide ? (
            <HttpCustomGuideCard guide={httpCustomGuide} copiedField={copiedHttpField} onCopy={copyHttpField} />
          ) : (
      ...
      <Dialog open={showEasyResult} onOpenChange={setShowEasyResult}>
        <DialogContent className="sm:max-w-lg glass-panel border-emerald-500/30">
          <DialogHeader>
            ...
            <DialogDescription>Unduh file .dark lalu buka atau import di aplikasi DarkTunnel.</DialogDescription>
          </DialogHeader>
          {easyResult && (
