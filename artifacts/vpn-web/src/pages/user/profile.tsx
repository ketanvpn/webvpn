...
import {
  ...
  CheckCircle2, Info, Zap, Crown, Target, Trophy, Flame, AlertCircle,
} from "lucide-react";
...
export default function Profile() {
  ...
  // === Bot VPN linkage state (terpisah dari Bot Notifikasi) ===
  // vpnTelegramId di tabel users → linked status; pakai fetch langsung karena
  // endpoint /telegram/vpn-link belum ada di OpenAPI auto-gen client.
  const [vpnTgInfo, setVpnTgInfo] = useState<{ vpnTelegramId: number | null }>({ vpnTelegramId: null });
  ...
  type PromoData = { promoEnabled: boolean; promoTitle: string; promoText: string; requestEnabled: boolean; discountPercent: number; autoUpgradeEnabled: boolean; autoUpgradeMinTopup: number; targetEnabled: boolean; monthlyTarget: number };
  ...
  // === Bot VPN linkage handlers ===
  // Fetch /auth/me untuk dapat vpnTelegramId field. /auth/me adalah endpoint
  // session-info yang mengembalikan info user (termasuk telegramId & vpnTelegramId).
  ...
  return (
    <div className="max-w-lg mx-auto pb-6 space-y-4">
      ...
      {/* ── Bot VPN (Akun Telegram untuk jualan/beli VPN) ── */}
      <Card className="glass-panel border-white/5 overflow-hidden shadow-sm">
        <div className="px-5 py-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1">
              <p className="font-semibold text-sm">Akun Bot VPN</p>
              <p className="text-xs text-muted-foreground">Sinkronkan saldo & akun lewat Bot VPN</p>
            </div>
            {vpnTgInfo.vpnTelegramId && (
          ...
          {vpnTgInfo.vpnTelegramId ? (
            <div className="rounded-lg bg-green-50 border border-green-200 px-3 py-2.5 flex items-center justify-between">
              <p className="text-xs text-green-700">ID: {vpnTgInfo.vpnTelegramId}</p>
              <button
                onClick={handleUnlinkVpn}
                disabled={vpnUnlinkLoading}
                className="text-xs text-red-500 hover:text-red-700 font-medium disabled:opacity-50"
              >
                {vpnUnlinkLoading ? "Memutus..." : "Putus koneksi"}
              </button>
            </div>
          ) : !vpnLink ? (
            <Button
              variant="outline"
              size="sm"
              onClick={handleGetVpnLink}
              disabled={vpnLinkLoading}
              className="gap-2 w-full"
            >
              <Send className="h-4 w-4" />
              {vpnLinkLoading ? "Membuat link..." : "Hubungkan ke Bot VPN"}
            </Button>
          ) : (
            <div className="space-y-2">
              <p className="text-xs text-muted-foreground">
                Klik link berikut untuk menghubungkan akun ke Bot VPN:
              </p>
              <a
                href={vpnLink}
                target="_blank"
      ...
      {/* ── Telegram Notifikasi (Bot Notifikasi yang sudah ada — JANGAN dihapus) ── */}
      <Card className="glass-panel border-white/5 overflow-hidden shadow-sm">
        <div className="px-5 py-4">
          <div className="flex items-center gap-3 mb-3">
      ...
      {user.role === "reseller" && resellerStatus && (() => {
        const progress = resellerStatus.progressPercent ?? 0;
        ...
        return (
          <div className="relative overflow-hidden rounded-xl border border-emerald-500/30 bg-gradient-to-br from-emerald-950/40 via-slate-900 to-slate-950 shadow-lg">
            ...
            <div className="relative px-5 py-4">
              <div className="flex items-start justify-between mb-4">
                ...
                <button
                  ...
                  className="text-muted-foreground hover:text-emerald-400 disabled:opacity-40 transition-colors"
                  title="Refresh"
                >
              ...
              <div className="flex items-center justify-between rounded-xl bg-emerald-500/10 border border-emerald-500/20 px-4 py-3 mb-4">
                ...
                <span className="font-bold text-emerald-400 text-xl">{resellerStatus.discountPercent}%</span>
              </div>
              ...
              {resellerStatus.targetEnabled ? (
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-bold text-white">{formatRupiah(resellerStatus.currentMonthSales)}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-muted-foreground">Target: {formatRupiah(resellerStatus.monthlyTarget)}</span>
                    </div>
                    {targetMet ? (
                  ...
                  {!targetMet && resellerStatus.monthlyTarget - resellerStatus.currentMonthSales > 0 && (
                    <div className="flex items-center gap-2 p-2.5 rounded-lg bg-orange-500/10 border border-orange-500/20">
                      ...
                      <p className="text-[11px] text-orange-300">
                        Kurang <span className="font-bold">{formatRupiah(resellerStatus.monthlyTarget - resellerStatus.currentMonthSales)}</span> untuk mencapai target
                      </p>
                    </div>
                  )}
                </div>
      ...
      {user.role === "user" && promo?.promoEnabled && (() => {
        ...
        return (
          <Card className={`overflow-hidden border shadow-sm relative ${
            ...
            {/* Dekorasi */}
            <div className={`absolute -right-6 -top-6 h-24 w-24 rounded-full blur-2xl opacity-50 ${isAuto ? "bg-green-200" : "bg-primary/20"}`} />
            ...
            <div className="relative px-5 pt-4 pb-5 space-y-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                  isAuto ? "bg-green-600 text-white" : "bg-primary text-primary-foreground"
                }`}>
              ...
              <div>
                <p className={`text-[10px] font-semibold uppercase tracking-wide mb-1.5 ${
                  isAuto ? "text-green-700 dark:text-green-400" : "text-primary"
                }`}>Yang kamu dapat</p>
